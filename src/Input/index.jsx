...
display: flex;
align-items: center;
padding: var(--aiwizo-application-spacing-mini) var(--aiwizo-application-spacing-small);
...
const Input = ({label, icon, ...props}) => {
  ...
  useEffect(() => {
    ...
    return (
      <Wrapper onClick={defer(handleFocus, inputRef, setFocus)} onBlur={defer(setFocus, false)}>
        {label ? <Label>{label}</Label> : null }
        <InputWrapper  focus={focus}>
        <TypeIcon icon={icon} focus={focus}/>
        <StyledInput ref={inputRef} {...props} value={value} onChange={targetValue(setValue)}/>
